...
Following the best-practice principle of DRY code, we use factories to define the creation of Epilepsy12 `User` instances, with default values set once, minimising repeated code. Then, we represent multiple different Epilepsy12 `User` types, such as a normal user (`e12User_GOSH`) or a superuser (`e12User_GOSH_superuser`), as "fixtures". These fixtures can be passed into tests, enabling faster and more consistent test-driven development.
